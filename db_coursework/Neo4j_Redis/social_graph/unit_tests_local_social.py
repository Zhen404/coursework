...
fg = fct.FanGraph(auth=('neo4j','neo4jdb'),
                              host="localhost",
                              port=7687,
                              secure=False)
...
def test_create_comment3():
    ...
    tid = t['team_id']
    fid = f['uni']
    c = fg.create_comment(fid, c, tid)
test_create_comment()
# test_create_comment2()
# test_create_comment3()

def test_create_sub_comment():
    c = fg.get_comment("305900b6-b684-43a5-9851-1aa28892f672")
    m = "Totally agree!"
    r = fg.create_sub_comment('al1', "305900b6-b684-43a5-9851-1aa28892f672", m)

#test_create_sub_comment()


def test_get_player_comment():
    c_result = fg.get_player_comments(player_id='pedrodu01')
    result = c_result.data()
    print(json.dumps(result, indent=2))

#test_get_player_comment()


def test_get_team_comment():
    c_result = fg.get_team_comments(team_id='BOS')
    result = c_result.data()
    print(json.dumps(result, indent=2))

#test_get_team_comment()

def test_get_team_comment2():
    c_result = fg.get_team_comments(team_id='WAS')
    result = c_result.data()
    print(json.dumps(result, indent=2))

#test_get_team_comment2()
